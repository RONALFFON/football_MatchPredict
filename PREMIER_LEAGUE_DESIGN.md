...
`backend/ai_service/` 独立包收纳全部 AI 能力：`llm.py`（OpenAI 兼容客户端）、`predictor.py`（五大联赛预测）、`agent/`（编排/工具/护栏）。通过**依赖倒置**解耦：AI 层定义 `PlDataProvider` 接口，app 层以 `app/pl_data/provider.py` 实现并注入；`ai_service` 对 app/scripts/数据库零 import，可独立测试与复用。
...
| 数据脚本耦合 | 五大联赛特征数据、体彩爬虫、模型调用混在同一 `scripts/` 目录 | 英超专项深化（xG、阵容、伤病、实时比分）无处安放 |
...
4. 可复用主站已有资产：用户体系、数据管道经验、统一模型服务接入。
...
| 候选 | 优势 | 劣势 | 结论 |
|------|------|------|------|
| **自研轻量 ReAct 循环 + OpenAI 兼容工具调用** | 依赖少、可控、贴合 Vercel 部署、便于定制护栏 | 需自己写编排 | ✅ **选定** |
...
用户提问
   │
   ▼
Orchestrator ──► OpenAI 兼容 Tool Calling
   │                  │ 返回 tool_call
   │            ┌─────┴──────────────────────────────┐
   │            ▼          ▼          ▼               ▼
...
环境变量清单（新应用）：`DB_HOST/PORT/NAME/USER/PASS`（建议换用 Secret 管理，禁止硬编码）、`AI_MODE`、`AI_API_KEY`、`AI_MODEL`、`AI_BASE_URL`、`JWT_SECRET`、`FOOTBALL_DATA_API_KEY`、`CORS_ORIGINS`。
